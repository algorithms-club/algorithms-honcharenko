...
class unionFindWithWeight {
	...
	connect(a, b) {
		...
		if (isNaN(this._elements[a]) && isNaN(this._elements[b])) {
			...
			if (this._weigthOfComponents[rootB]) {
				this._weigthOfComponents[rootB] = this._weigthOfComponents[rootB] + 1;
			} else {
				this._weigthOfComponents[rootB] = 2
			}
		} else if (isNaN(this._elements[b])) {
			/* if component b hasn't root */
			let rootA = this.getMainRootElement(a);
			this._elements[b] = rootA;
			if (this._weigthOfComponents[rootA]) {
				this._weigthOfComponents[rootA] = this._weigthOfComponents[rootA] + 1;
			} else {
				this._weigthOfComponents[rootA] = 2;
			}
		} else {
